...
import { PayPalOrderStatusResponse } from '@/app/interfaces/paypal';
...
export const paypalCheckPayment = async (transactionId: string) => {
  ...
  if (status !== 'COMPLETED') {
    return {
      ok: false,
      msg: 'Todavia no se pago en paypal'
    }
  }
  ...
  try {
    ...
    return {
      ok: false,
      msg: 'error verificando pago en bd'
    }
  }
...
const verifyPayPalPayment = async (
  ...
  try {
    const results = await fetch(orderUrl, { ...requestOptions, cache: 'no-store'}).then((r) => r.json());
    return results;
  } catch (error) {
    console.log(error);
    return null;
